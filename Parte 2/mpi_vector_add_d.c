...

{
    ...
    double result = Parallel_point_product(local_x, local_y, local_n);

    double final_res;

    MPI_Reduce(&result, &final_res, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

    /* Parallel_scalar_product(local_x, 10, local_n);

    Parallel_scalar_product(local_y, 10, local_n);
